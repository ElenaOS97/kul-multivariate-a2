...
```{r Task_2_13Y, comment=NA, warning=FALSE, eval=FALSE}
# (3) random forests
set.seed(0829539)
rf.mod=randomForest(spam ~ .,data=data.train, classwt=c(0.606,0.394), mtry=5, 
                    ntree=nrotree, importance=TRUE)
rf.mod
```
...
pred.train<-predict(T2_ranfor_2,newdata=data.train)
err(data.train$spam,pred.train)
