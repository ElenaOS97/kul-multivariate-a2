...
1.1 ratio
```{r Task_5_1, include=TRUE}
m1 <- smacofSym(delta=dissim, ndim=2, type="ratio", init="torgerson")

```
1.2 interval 
```{r Task_5_2, include=TRUE}
m2 <- smacofSym(delta=dissim, ndim=2, type="interval", init="torgerson")
```

1.3 mspline
```{r Task_5_3, include=TRUE}
m3<-smacofSym(delta=dissim, ndim=2, type="mspline", spline.degree =4, spline.intKnots = 4, init="torgerson")
```

1.4 ordinal
```{r Task_5_4, include=TRUE}
m4 <- smacofSym(delta=dissim, ndim=2, type="ordinal", init="torgerson")
```

```{r Task_5_5, include=TRUE}
...
another_matrix <- data.frame(v_signal) %>% 
  mutate(alph = alphabet, 
